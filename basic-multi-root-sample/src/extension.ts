...
export function activate(context: ExtensionContext) {
	...
	// context.subscriptions.push(window.onDidChangeTextEditorViewColumn(e => {
	// 	updateStatus(status);
	// }));
...
function updateStatus(status: StatusBarItem): void {
	...
	status.text = info ? info.text || 'dddd' : '顶顶顶顶';
	status.tooltip = info ? info.tooltip : undefined;
	status.color = info ? info.color : '#ff0000';
...
function getEditorInfo(): { text?: string; tooltip?: string; color?: string; } | null {
	...
	if (!editor || !workspaceFolders || workspaceFolders.length < 2) {
		return null;
	}
	...
	if (resource.scheme === 'file') {
		...
		if (!folder) {
			...
			text = `$(file-submodule) ${basename(folder.uri.fsPath)} (${folder.index + 1} of ${workspaceFolders.length}) → $(file-code) ${basename(resource.fsPath)}`;
